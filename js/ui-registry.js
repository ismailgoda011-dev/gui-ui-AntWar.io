...
export function renderIconButton(item, options = {}) {
    ...
    const labelId = item.labelId || `${item.id}-label`;
    const iconId = item.iconId || `${item.id}-icon`;
    return `<button id="${item.id}" class="${buttonClass}" data-ui-action="${item.action || item.modal || ''}" aria-label="${label}" title="${title}" type="button"><div class="${iconWrapClass}"><img id="${iconId}" src="${icon}" alt="${label}" class="${iconClass} ${iconExtra}">${badge}${levelTag}</div><span id="${labelId}" class="${labelClass}">${label}</span></button>`;
}
...
export function renderBottomNav(container) {
    if (!container || !registry) return;
    container.innerHTML = (registry.bottomNav || []).map((item, index) => `<button class="bnav-item ${index === 0 ? 'active' : ''}" id="${item.id}" data-action="${item.action || ''}" data-label-key="${item.labelKey || ''}" type="button" aria-label="${getLabel(item, item.id)}"><div class="bnav-icon-wrap"><img src="${asset(item.icon)}" alt="${getLabel(item, item.id)}" class="${item.iconClass || ''}"></div><span class="bnav-label">${getLabel(item, item.id)}</span></button>`).join('');
}

export function renderServerOptions(container) {
    if (!container || !registry) return;
    const servers = registry.serverOptions || [];
    container.innerHTML = servers.map(server => `<button type="button" class="server-option-btn w-full px-1.5 py-1 rounded flex items-center justify-between hover:c-bg-burgundy transition text-right cursor-pointer" data-server-id="${server.id}" data-ping="${server.ping}" data-status="${server.status}"><span class="text-[10px] font-bold text-white flex items-center gap-1">${server.flag || ''} ${t(server.nameKey)}</span><span class="text-[9px] font-black ${server.status === 'emerald' ? 'text-[#37AA49]' : 'text-[#FFD875]'}">${server.ping}</span></button>`).join('');
    return servers;
}
