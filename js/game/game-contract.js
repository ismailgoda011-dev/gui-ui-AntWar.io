...
export const GAME_CONTRACT = Object.freeze({
  ...
  optional: ['startSingleplayer','getState','getPlayer','getRoom','getMatch','getInventory','getConnection','joinRoom','leaveRoom','setReady','matchmake','cancelMatchmaking','on','off'],
  events: ['state:changed','player:changed','room:changed','match:changed','inventory:changed','connection:changed','game:started','game:ended','game:error','room:joined','room:left','player:ready','match:found']
});
...
export function validateGameEngine(engine = window.game) {
  ...
  const available = GAME_CONTRACT.optional.filter(name => typeof engine[name] === 'function');
